...
class ModisMap:
    ...
    def set_startend_position(self, start_position, end_position):
        ...
        self.start_point = (int(start_position[0] * self.h), int(start_position[1] * self.w))
        self.end_point = (int(end_position[0] * self.h), int(end_position[1] * self.w))
    ...
    def __get_sea_probability(self, x, y):
        return self.prob[y - self.prob_y_start, x - self.prob_x_start, 0]

    def __get_thin_ice_probability(self, x, y):
        return self.prob[y - self.prob_y_start, x - self.prob_x_start, 1]

    def __get_thick_ice_probability(self, x, y):
        return self.prob[y - self.prob_y_start, x - self.prob_x_start, 2]

    def __get_probability_by_point(self, point, index):
        return self.prob[point[1] - self.prob_y_start, point[0] - self.prob_x_start, index]
                    ...
                    if p1 not in self.infeasible_set and p2 not in self.infeasible_set:
                        ...
                        if not self.__is_in(p1[0], p1[1]):  # out of range
                            ...
                            p1_cost = ModisMap.PIXEL_RATIO * self.__get_target_cost(p1[0], p1[1]) + \
                                      (1 - ModisMap.PIXEL_RATIO) * self.__get_thick_ice_probability_by_point(
                                          p1) * ModisMap.PROB_ENLAGRED_TIMES
                        if not self.__is_in(p2[0], p2[1]):
                            ...
                            p2_cost = ModisMap.PIXEL_RATIO * self.__get_target_cost(p2[0], p2[1]) + \
                                      (1 - ModisMap.PIXEL_RATIO) * self.__get_thick_ice_probability_by_point(
                                          p2) * ModisMap.PROB_ENLAGRED_TIMES
